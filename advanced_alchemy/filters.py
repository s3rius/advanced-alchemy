...
class Cursor(PaginationFilter):
    ...
    def append_to_statement(
        ...
        if isinstance(statement, Select):
            ...
            new_statement = statement.order_by(*self._order_by_clause(statement, field, model)).limit(self.limit)
    ...
    def _order_by_clause(
        ...
        if field.key is None:
            msg = "Cannot get order_by field name"
            raise ValueError(msg)

        # We don't care much about pk ordering, we
        # just want it to be the same.
        pks = [pk.asc() for pk in model.__table__.primary_key if pk.key != field.key]

        if self.sort_order == "asc":
            return [field.asc(), *pks]
        return [field.desc(), *pks]
    ...
    def _keyset_condition(
        ...
        for pk_column, pk_value in zip(pk_columns, pk_values):
            # We skip primary keys which
            # happen to be in the query already.
            if pk_column.key == field.key:
                continue
            # Since PKs are always ascending, we can just use gt.
